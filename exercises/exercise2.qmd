...
series of numbers, letters, or other data elements. An alternative solution is
`vec1 <- 1:6`
`vec2 <- LETTERS[1:6]`
```
...
1. Use the `as.data.frame()` function with the matrix created in 
@cref-matrix as input.
...
**Check if the first columns in the data frames from @cref-df1 and 
@cref-df2 are numeric** 
...
```{asis, echo = params$answers}
The first column in `dat3b` (from @cref-df2 is not numeric. As a 
...
**Select the following elements from the data frame you created in @cref-df1**
...
**Inspect the structure of the data frame that you created in @cref-df1.**
...
@cref-df1 (`dat3`) is not coded correctly, and it actually represents grouping 
